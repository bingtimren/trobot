...
describe("", () => {
    ...
    test("Place robot on table and move around", () => {
        ...
        expect(robot.placeOn(table, pose)).toEqual(pose);
        expect(robot.getPose()).toEqual(pose);
        expect(robot.moveForward()).toEqual({ x: 1, y: 0, facing: MapDirection.EAST })
        expect(robot.turnLeft()).toEqual({ x: 1, y: 0, facing: MapDirection.NORTH })
        expect(robot.moveForward()).toEqual({ x: 1, y: 1, facing: MapDirection.NORTH })
        expect(robot.turnRight()).toEqual({ x: 1, y: 1, facing: MapDirection.EAST })
        expect(robot.moveForward()).toEqual({ x: 2, y: 1, facing: MapDirection.EAST })
    })
    test("Robot ignores invalid commands", () => {
        expect(robot.placeOn(table, { x: -1, y: -1, facing: MapDirection.EAST })).toBeInstanceOf(Error)
        ...
        expect(robot.moveForward()).toBeInstanceOf(Error);
        expect(robot.getPose()).toEqual(pose);
        expect(robot.turnLeft()).toEqual({ x: 0, y: 0, facing: MapDirection.SOUTH })
        expect(robot.moveForward()).toBeInstanceOf(Error);
        ...
        expect(robot.moveForward()).toEqual({ x: 1, y: 0, facing: MapDirection.EAST });
        expect(robot.moveForward()).toEqual({ x: 2, y: 0, facing: MapDirection.EAST });
        expect(robot.moveForward()).toEqual({ x: 3, y: 0, facing: MapDirection.EAST });
        expect(robot.moveForward()).toEqual({ x: 4, y: 0, facing: MapDirection.EAST });
        expect(robot.moveForward()).toBeInstanceOf(Error);
        expect(robot.getPose()).toEqual({ x: 4, y: 0, facing: MapDirection.EAST });
        expect(robot.turnLeft()).toEqual({ x: 4, y: 0, facing: MapDirection.NORTH })
        expect(robot.moveForward()).toEqual({ x: 4, y: 1, facing: MapDirection.NORTH });
        expect(robot.moveForward()).toEqual({ x: 4, y: 2, facing: MapDirection.NORTH });
        expect(robot.moveForward()).toBeInstanceOf(Error)
