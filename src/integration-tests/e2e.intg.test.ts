...
const e2eData: { desc: string; commands: string[]; outputs: RegExp[] }[] = [
  ...
  },
  {
    ...
    commands: [
      "MOVE",
      "REPORT", // bumble at left bottom corner
      "LEFT",
      "REPORT",
      "RIGHT",
      "REPORT",
      ...
      "MOVE",
      "MOVE",
      "REPORT", // bumble to 0,0
    ],
    outputs: [
      WELCOME,
      new RegExp(
        `^${PROMPT}${PROMPT}${PROMPT}${PROMPT}${PROMPT}${PROMPT}${PROMPT}${PROMPT}${PROMPT}0,0,WEST$`
      ),
      new RegExp(`^${PROMPT}${PROMPT}${PROMPT}0,0,SOUTH$`),
      new RegExp(
        `^${PROMPT}${PROMPT}${PROMPT}${PROMPT}${PROMPT}${PROMPT}${PROMPT}4,0,EAST$`
      ), // to right bottom
      new RegExp(`^${PROMPT}${PROMPT}4,0,EAST$`),
      new RegExp(`^${PROMPT}${PROMPT}4,0,EAST$`),
      new RegExp(
        `^${PROMPT}${PROMPT}${PROMPT}${PROMPT}${PROMPT}${PROMPT}${PROMPT}4,4,NORTH$`
      ),
      new RegExp(`^${PROMPT}${PROMPT}${PROMPT}4,4,EAST$`),
      new RegExp(
        `^${PROMPT}${PROMPT}${PROMPT}${PROMPT}${PROMPT}${PROMPT}${PROMPT}${PROMPT}0,4,WEST$`
      ),
      new RegExp(`^${PROMPT}${PROMPT}${PROMPT}0,4,NORTH$`),
      new RegExp(
        `^${PROMPT}${PROMPT}${PROMPT}${PROMPT}${PROMPT}${PROMPT}${PROMPT}${PROMPT}0,0,SOUTH$`
      ),
      new RegExp(`^${PROMPT}$`),
      BYE,
    ],
...
async function testRun(
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    readLineInstance.on("line", (line: string) => {
      const expected =
        outputLineCount < expectedOutput.length
          ? expectedOutput[outputLineCount]
          : undefined;
      if (expected && expected.test(line)) {
        report.push(`LINE ${outputLineCount}: ${line} (ok)`);
      } else {
        const message = `LINE ${outputLineCount}: ${line} (ERROR: not match ${
          expected ? expected.source : "UNEXPECTED"
        })`;
        report.push(message);
