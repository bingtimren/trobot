...
function addNewlineToEndOfString(s: string): string { return s.endsWith("\n") ? s : s + "\n" }

export class BaseStreamConsole {
    private readline: readline.Interface;
    private _inputClosed: boolean;
    private resolvers: (() => void)[] = [];
    get isInputClosed(): boolean { return this._inputClosed };
    ...
    constructor(input: Readable, output: Writable, error: Writable, private handler: CommandLineHandler, options: {
        prompt?: string,
        greetingMessage?: string,
        farewellMessage?: string
    } = {}) {
        this.readline = readline.createInterface({ input, output, prompt: options.prompt });
